        ...
        for product in state.order_depths.keys():
           ...
           # VOLATILE COMMODITIES => |slope| over 3.67722
            if product == 'DIVING_GEAR':
                Position_Limit = 50
                acceptable_bid_price = (middle_bid + best_ask)/2
                acceptable_ask_price = (best_bid + middle_ask)/2   
            elif product == 'UKELELE':
                Position_Limit = 50
                if (best_ask + best_bid)/2 >= 21000:
                    acceptable_bid_price = (middle_bid + best_ask)/2
                    acceptable_ask_price = (best_bid + middle_ask)/2   
                else:
                    acceptable_bid_price = (best_bid + best_ask)/2
                    acceptable_ask_price = (best_bid + best_ask)/2 
            elif product == 'PICNIC_BASKET':
                Position_Limit = 250
                if (best_ask + best_bid)/2 >= 75000:
                    acceptable_bid_price = (middle_bid + best_ask)/2
                    acceptable_ask_price = (best_bid + middle_ask)/2   
                else:
                    acceptable_bid_price = (best_bid + best_ask)/2
            ...
            # INVOLATILE COMMODITIES => |slope| under 3.67722
                ...
                if (best_ask + best_bid)/2 >= 10000:
                    acceptable_bid_price = (worst_bid + best_ask)/2
                    acceptable_ask_price = (best_bid + worst_ask)/2   
                else:
                    acceptable_bid_price = (middle_bid + best_ask)/2
                    acceptable_ask_price = (best_bid + middle_ask)/2
